...
// Estado
  ...
  // Modales
  abrirEntrega(titulo: string, curso: string) {
    this.modalTareaTitle = titulo;
    this.modalTareaCurso = curso;
    this.showModalEntrega = true;
  }
  ...
  confirmarEntrega() {
    this.cerrarModal();
    this.showToast('¡Tarea entregada correctamente!', 'success');
  }

  publicarForo() {
    this.cerrarModal();
    this.showToast('Publicación enviada al foro', 'success');
  }

  // Utilidad para clase de nota
  getNotaClass(n: string | number) {
    const v = parseFloat(n as string);
    if (isNaN(v)) return '';
    if (v >= 17) return 'nota-a';
    if (v >= 14) return 'nota-b';
    if (v >= 11) return 'nota-c';
    return 'nota-d';
  }
